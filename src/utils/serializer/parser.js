...
Lexer.prototype.token = function(src, top, bq) {
    ...
    while (src) {
        ...
        if ((cap = this.rules.code.exec(src))) {
            src = src.substring(cap[0].length);
            console.log('CODE IS HERE', cap[0]);
...
InlineLexer.prototype.parse = function(src) {
    ...
    while (src) {
        ...
        if ((cap = this.rules.underlined.exec(src))) {
            src = src.substring(cap[0].length);
            out.push(this.renderer.underlined([this.renderer.text(cap[0])]));
            continue;
        }
        ...
        if ((cap = this.rules.strong.exec(src))) {
            src = src.substring(cap[0].length);
            out.push(this.renderer.strong([this.renderer.text(cap[0])]));
            continue;
        }
        ...
        if ((cap = this.rules.em.exec(src))) {
            src = src.substring(cap[0].length);
            out.push(this.renderer.em([this.renderer.text(cap[0])]));
            continue;
        }
        ...
        if ((cap = this.rules.code.exec(src))) {
            src = src.substring(cap[0].length);
            out.push(this.renderer.codespan(cap[0]));
            continue;
        }
