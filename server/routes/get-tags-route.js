export default async function (ctx, next) {
    const { data } = ctx;
    const tags = await data.sequelize.query('select regexp_matches(text, \'\\#\\w+\', \'g\') as tags from list_items where text ~ \'\\#\\w+\' order by tags', { type: data.sequelize.QueryTypes.SELECT });
